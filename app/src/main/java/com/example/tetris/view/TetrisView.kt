...
class TetrisView : View {
            ...
            if (message.what == 0) {
                if (owner.model != null) {
                    if (owner.model!!.isGameOver()) {
                        owner.model?.endGame()
                        owner.activity?.b?.tvCurrentScore?.text = "0"
    ...
    private fun updateScores() {
        ...
        activity?.b?.tvHighScore?.text = "${activity?.appPreferences?.getHighScore()}"
    ...
    private fun drawCell(canvas: Canvas, row: Int, col: Int) {
        ...
        if (CellConstants.EMPTY.value != cellStatus) {
            val color = if (CellConstants.FILLED.value == cellStatus) {
